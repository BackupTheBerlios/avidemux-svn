...
mpegWritter::save_svcd (const char *name)
{
  ...
  switch (SVCDCodec.mode)
    {
    case COMPRESS_CBR:
      return save_regular (name, ADM_SVCD, 0, SVCDCodec.bitrate, SVCDExtra.user_matrix, SVCDExtra.interlaced, SVCDExtra.bff,	// WLA
			   SVCDExtra.widescreen);
      break;
    case COMPRESS_CQ:
      return save_regular (name, ADM_SVCD, SVCDCodec.qz, 8 * SVCDExtra.maxBitrate, SVCDExtra.user_matrix, SVCDExtra.interlaced, SVCDExtra.bff,	// WLA
			   SVCDExtra.widescreen);
      break;
    case COMPRESS_2PASS:
      return save_dualpass (name, SVCDCodec.finalsize, 8 * SVCDExtra.maxBitrate, ADM_SVCD, SVCDExtra.user_matrix, SVCDExtra.interlaced, SVCDExtra.bff,	// WLA
			    SVCDExtra.widescreen);
      break;
    }
  ADM_assert (0);
...
mpegWritter::save_dvd (const char *name)
{
  ...
  switch (DVDCodec.mode)
    ...
    case COMPRESS_CBR:
      ...
      return save_regular (name, ADM_DVD, 0, DVDCodec.bitrate, DVDExtra.user_matrix, DVDExtra.interlaced, DVDExtra.bff,	// WLA
			   DVDExtra.widescreen);
      break;
    case COMPRESS_CQ:
      printf ("Dvd encoding in CQ mode\n");
      return save_regular (name, ADM_DVD, DVDCodec.qz, 8 * DVDExtra.maxBitrate, DVDExtra.user_matrix, DVDExtra.interlaced, DVDExtra.bff,	// WLA
			   DVDExtra.widescreen);
      break;
    case COMPRESS_2PASS:
      printf ("Dvd encoding in 2pass mode\n");
      return save_dualpass (name, DVDCodec.finalsize, 8 * DVDExtra.maxBitrate, ADM_DVD, DVDExtra.user_matrix, DVDExtra.interlaced, DVDExtra.bff,	// WLA
			    DVDExtra.widescreen);
      break;
    }
  ADM_assert (0);
...
uint8_t
mpegWritter::save_vcd (const char *name)
{
...
uint8_t
mpegWritter::init (const char *name, ADM_MPEGTYPE type, uint8_t interlaced, uint8_t bff, uint8_t widescreen)	// WLA
{
  UNUSED_ARG (name);
  UNUSED_ARG (type);
...
//
uint8_t
mpegWritter::initLveMux (const char *name, ADM_MUXER_TYPE type)
{
