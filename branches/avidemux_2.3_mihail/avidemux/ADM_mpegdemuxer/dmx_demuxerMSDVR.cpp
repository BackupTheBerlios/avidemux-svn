...
//
 uint8_t         dmx_demuxerMSDVR::getPacketInfo(uint8_t **data, uint32_t *len, uint32_t *usableLen,uint64_t *pts)
{
  *data=_pesBuffer;
  *len=_pesBufferLen;
...
#define INSIDE(min,max) (stream>=min && stream<max)
      ...
      //  seen[globstream & 0xFF]+=len;
        if(myPid==globstream)
        {
          ...
          {
            printf("Read failed dmx_demuxerMSDVR::refill %d\n",len);
            return 0;
          }
          return 1;
        }
...
****************************************************/
uint8_t dmx_demuxerMSDVR::refillFull(uint8_t *outstream)
{
  uint32_t globstream,len;
  uint8_t  stream,substream;
  uint64_t abs,pts,dts;
...
uint8_t dmx_demuxerMSDVR::getPacketInfo(uint8_t stream,uint8_t *substream,uint32_t *olen,uint64_t *opts,uint64_t *odts)
{
