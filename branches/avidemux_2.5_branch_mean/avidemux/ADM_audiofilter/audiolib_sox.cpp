...
static void LpFilter(double c[],
		     ...
		     double Beta,
		     long Num);
static long st_gcd(long a, long b);
		...
		uint32_t inc)
{
	...
	if (Nx <= 0)
	{
		printf("resample: Can not handle this sample rate change. Nx not positive: %d", Nx);
		return (0);
	}
	if (Nx > *isamp)
		Nx = *isamp;
	...
	{ 
		...
		Float ftemp = r->Y[i];  // 32 bits -> 16 bits MEANX
...
*/
#if 0
int sow_drain(ResampleStruct *r, 		
		int16_t *obuf,                
		uint32_t *osamp)
{
	
	uint32_t isamp_res, osamp_res;
	int16_t *Obuf;
	int rc;
	...
	while (isamp_res>0 && osamp_res>0) {
		...
		Isamp = isamp_res;
		Osamp = osamp_res;
		rc = sox_run(r, NULL, Obuf, (uint32_t *)&Isamp, (uint32_t *)&Osamp);
		if (!rc)
		    return rc;
	...
	*osamp -= osamp_res;
	...
	if (isamp_res)
		printf("drain overran obuf by %d\n", isamp_res); fflush(stderr);
	return (1);
}
