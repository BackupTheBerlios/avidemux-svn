...
static myENC_RESULT res;
	ffmpegEncoder::ffmpegEncoder(uint32_t width,uint32_t height,FF_CODEC_ID id)     : encoder(width,height)
             				{
		printf("\n Build : %d \n",LIBAVCODEC_BUILD);
...
*/
uint8_t ffmpegEncoder::getResult( void *ress)
{
	memcpy(ress,&res,sizeof(res));
   	return 1;
}

uint8_t ffmpegEncoder::encodePreamble( uint8_t *in )
{
       _frame.linesize[0]=_w;
       _frame.linesize[1]=_w>>1;
       _frame.linesize[2]=_w>>1;
...
uint32_t  ffmpegEncoder::frameType( void )
{
	int k,t;
...
*/
uint8_t     ffmpegEncoder::gopMpeg1(void)
{
...
uint8_t     ffmpegEncoder::initContext(  void )
{
	...
	// player compatiblity		     
		...
		switch(_id)
		{
			...
			case FF_MSMP4V3 : 	res=  avcodec_open(_context, &msmpeg4v3_encoder);break;
			...
			case FF_H263  : 		res=  avcodec_open(_context, &h263_encoder);break;
			case FF_H263P  : 	res=  avcodec_open(_context, &h263p_encoder);break;
...
*/
 uint8_t ffmpegEncoder::setCustomMatrices(uint16_t *intra, uint16_t *inter)
 {
		_context->intra_matrix=intra;
    		_context->inter_matrix=inter;
		return 1;
 }
 uint8_t  ffmpegEncoder::setGopSize(uint32_t size)
 {
...
uint8_t ffmpegEncoder::setLogFile(const char *name)
{
	strcpy(LogName,name);
	return 1;
	
}
uint8_t	ffmpegEncoder::setConfig(FFcodecSetting *set)
{
...
uint8_t ffmpegEncoderCQ::init( uint32_t val,	uint32_t fps1000,uint8_t vbr)
{
...
uint8_t ffmpegEncoderCQ::encode(	uint8_t 	*in,
				 	uint8_t 	*out,
					uint32_t 	*len,
					uint32_t 	*flags)
{
...
uint8_t ffmpegEncoderCBR::encode(	uint8_t 	*in,
					uint8_t 	*out,
					uint32_t 	*len,
			       		uint32_t 	*flags)
{
...
//		VBR
//------------------------------------------------------------------------------
uint8_t ffmpegEncoderVBR::encode(		uint8_t 	*in,
					 	uint8_t 	*out,
						uint32_t 	*len,
			       			uint32_t 	*flags)
{
...
uint8_t ffmpegEncoderVBR::encodeVBR(
			uint8_t * in,
		    	uint8_t * out,
			uint32_t * len,
       			uint32_t * flags,
...
*/
uint8_t ffmpegEncoderVBR::init( uint32_t val,uint32_t fps1000)
{
	uint32_t statSize;
	FILE *_statfile;
...
uint8_t ffmpegEncoderVBRExternal::encode(		uint8_t 	*in,
					 	uint8_t 	*out,
						uint32_t 	*len,
			       			uint32_t 	*flags)
{
...
uint8_t ffmpegEncoderVBRExternal::encodeVBR(
			uint8_t * in,
		    	uint8_t * out,
			uint32_t * len,
       			uint32_t * flags,
...
*/
uint8_t ffmpegEncoderVBRExternal::init( uint32_t val,uint32_t fps1000)
{
...
void ffmpegEncoder::mplayer_init(void )
{
	/*
...
uint8_t ffmpegEncoder::init( uint32_t val,uint32_t fps1000) {
    UNUSED_ARG(val);
    UNUSED_ARG(fps1000);
	return 0;
}

uint8_t ffmpegEncoder::init( uint32_t val,uint32_t fps1000,uint8_t sw) {
    UNUSED_ARG(sw);
	return init(val,fps1000);
}

uint8_t ffmpegEncoder::encode( uint8_t *in, uint8_t *out, uint32_t *len, uint32_t *flags) {
    UNUSED_ARG(in);
    UNUSED_ARG(out);
    UNUSED_ARG(len);
    UNUSED_ARG(flags);
...
//------------------------------
uint8_t ffmpegEncoderHuff::init( uint32_t val,	uint32_t fps1000,uint8_t vbr)
{
    UNUSED_ARG(val);
    UNUSED_ARG(vbr);
	mplayer_init();
...
uint8_t ffmpegEncoderHuff::encode(	uint8_t 	*in,
				 	uint8_t 	*out,
					uint32_t 	*len,
					uint32_t 	*flags)
{
...
uint8_t ffmpegEncoder::getExtraData(uint32_t *l,uint8_t **d)
{
...
//----
//------------------------------
uint8_t ffmpegEncoderFFV1::init( uint32_t val,	uint32_t fps1000,uint8_t vbr)
{
    UNUSED_ARG(val);
    UNUSED_ARG(vbr);
	mplayer_init();
...
uint8_t ffmpegEncoderFFV1::encode(	uint8_t 	*in,
				 	uint8_t 	*out,
					uint32_t 	*len,
					uint32_t 	*flags)
{
...
/*---
*/
uint8_t ffmpegEncoderFFMjpeg::init( uint32_t val,	uint32_t fps1000,uint8_t vbr)
{
    UNUSED_ARG(val);
    UNUSED_ARG(vbr);
	mplayer_init();
...
uint8_t ffmpegEncoderFFMjpeg::encode(	uint8_t 	*in,
				 	uint8_t 	*out,
					uint32_t 	*len,
					uint32_t 	*flags)
{
