...
uint8_t ADM_AC3Run(uint8_t * ptr, uint32_t nbIn, 
  ...
  *nbOut=0;
  // First copy in -> Buffer
   ...
   inbuffer+=nbIn;
    ...
    if((nbIn==0)&&(inbuffer<(3840+7))) return 0;

lop:
//	printf("\n %lu bytes in buffer",inbuffer);
    if(inbuffer<7) return 1;
    // assume it is synced
     ...
     if(length==0)
     	{
           	length=AC3_Resync();
            	if(length==0) // resync failed
             	{
                 	if(nbIn==0) return 0; // probably end of stream
                  	else
                   	return 1;
...
//  and now go PCM
        	...
        	for (i = 0; i < 6; i++)
           {
                     ...
                     else
                     {
                           ...
                           if(chan==2)
                           	*ptrOut++=(int16_t)ceil(*(ac3_sample+256+j));
              			}
                 			*nbOut+=256*2*chan;
              ...
              // we did not found sync till here
         		...
         		// stuff in buffer
                   ...
                   memcpy(sync,buffer+pos,7);
               	  if( (length=a52_syncinfo (sync,&flags, &samprate, &bitrate))>0)
                		{
