...
//MEANX
int mpeg2_cleanup(mpeg2dec_t * mpeg2dec);
...
#endif
{
    ...
    do {
	byte = *current++;
	if (shift == 0x00000100 && byte!=0xE0 )
	{ // MEANX: Avoid troubles with some DVB
	    
	    mpeg2dec->shift = 0xffffff00;
	    skipped = (unsigned long)current - (unsigned long)mpeg2dec->buf_start;
...
#endif
{
    ...
    do {
	byte = *current++;
	if (shift == 0x00000100 && byte!=0xE0) {//MEANX : avoid troubles with some DVB
...
/*********************** MEANX ******************
 Mean-- We force clean up of the current image
 no need to start a new image, because we *KNOW* that
 there is enougth data
*/
int mpeg2_cleanup(mpeg2dec_t * mpeg2dec)
{
	    mpeg2_slice (&(mpeg2dec->decoder), mpeg2dec->code,
