...
//MEANX
static const int reverse_non_linear_scale[113] =
{
...
/*--MEANX, retrieve Q scale used for this macroblock--*/
static void updateQscale (const mpeg2_decoder_t * decoder)
{
 ...
 offset=y*decoder->quant_stride+x;
	...
	if(decoder->quant )
	{
		if(decoder->qscaletype)
		{
			assert(q<113);
			decoder->quant[offset]=reverse_non_linear_scale[q];
		}
		else
		{
			assert(q<=62);
			decoder->quant[offset]=q>>1;
		}
	}
	//printf("<<x:%d y:%d qin:%d q:%d\n",x,y,q,decoder->quant[y*decoder->quant_stride+x]);
...
#define bit_ptr (decoder->bitstream_ptr)
	...
	// MEANX
    decoder->qz=quantizer_scale_code;
    // / MEANX
#undef bit_buf
#undef bits
#undef bit_ptr
